...
#[generate_trait]
impl LockImpl of LockTrait {
    #[inline(always)]
    fn new(game_id: u32, player: ContractAddress, locking_period: u32) -> Lock {
        // [Check] Host is valid
        assert(!player.is_zero(), errors::GAME_INVALID_HOST);

        Lock { game_id, player, unlock_time: locking_period }
    }
}
#[generate_trait]
impl LockAssert of AssertTrait {
    #[inline(always)]
    fn assert_can_paint(self: Lock, current_time: u32) {
        assert(self.unlock_time < current_time, errors::GAME_NOT_HOST);
    }

    #[inline(always)]
    fn assert_is_exists(self: Lock, current_time: u32) {
        assert(self.unlock_time != 0, errors::GAME_NOT_HOST);
    }
}
